...
function District({
  ...
  return (
    <div className="md:flex-1 h-full relative px-4 py-6 cursor-pointer delay-100 md:hover:flex-2 transition-all ease-in-out duration-500 group/district md:not-hover:brightness-40">
      <img
        src={img}
        alt={heading}
        className="object-cover h-full w-screen md:w-auto md:h-[80vh] absolute inset-0 -z-10 brightness-40 group-hover:brightness-35 transition-all duration-500"
      />
      <div className="flex gap-2 items-center">
        <div className="w-1 self-stretch bg-primary"></div>
        <h1 className="text-3xl font-medium">{heading}</h1>
      </div>
      <p className="text-xs md:text-lg md:mt-9 md:opacity-0 md:group-hover/district:opacity-100 md:group-hover/district:translate-y-0 md:translate-y-6 md:text-center transition-all duration-500 md:group-hover/district:delay-300 ease-in-out px-9 py-5">
        {paragraph}
      </p>
    </div>
  );
